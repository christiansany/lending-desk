...
describe('planChaos — the pure function behind panel and tests', () => {
  ...
  test('flaky fails every third request', () => {
    const statuses = [1, 2, 3, 4, 5, 6].map(
      (requestCount) => planChaos({ ...base, switches: ['flaky'], requestCount }).response?.status ?? 200,
    )
  ...
  test('error:items only hits the item endpoints', () => {
    ...
    expect(planChaos({ ...base, switches: ['error:items'], endpoint: 'availability' }).response?.status).toBe(500)
    expect(planChaos({ ...base, switches: ['error:items'], endpoint: 'reservations' }).response).toBeNull()
  })
