...
export function parseItemQuery(url: URL): ItemQuery {
  ...
  if (rawStatus !== null && rawStatus !== '' && !STATUS_VALUES.includes(rawStatus as StatusFilter)) {
...
export function validateItem(input: ItemInput): Record<string, string> {
  ...
  if (typeof input.condition !== 'string' || !CONDITIONS.includes(input.condition as Item['condition'])) {
    errors.condition = 'Please pick a condition'
  }
  if (typeof input.dailyRate !== 'number' || !Number.isFinite(input.dailyRate) || input.dailyRate < 0) {
