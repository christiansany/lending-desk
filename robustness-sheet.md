...
> _observed_, not what you expected.
...
| E2  | Errors        | Chaos `409` on, submit the form                               | _What_ is taken and until when                        |          |          |             |
...
| Z2  | Emptiness     | Filter `owner=mine` + `status=reserved` until nothing matches | An empty state that says _which_ filters are to blame |          |          |             |
